...
`ui/designsystem/` is the only place a styling constant may exist (color/typography/spacing/corner/elevation tokens, `WaldoTheme`, twelve stateless presentational components as of A2). Screens compose those components and are driven by ViewModels; nothing outside `ui/designsystem/token/` hardcodes a `Color`, `.dp`, or `.sp`. Both light and dark token sets ship from day one. See specs/003 §4 for the full token vocabulary and default values, and `ui/designsystem/components/ComponentGalleryPreview.kt` for a side-by-side preview of every component in both themes.
