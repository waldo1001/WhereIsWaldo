...
3. **SMS region policy — allowlist: BE, NL, FR, DE, LU** (product-owner decision, 2026-07-21). This is the primary SMS-pumping / toll-fraud guardrail; a country is added only when a real user needs it, never preemptively.
...
- v1 relies on (and documents here as the deliberate posture): Firebase's built-in per-number/per-device/per-project SMS quotas and abuse detection; device attestation via App Check (§6.5); and the **SMS region allowlist** (§6.3) bounding the toll-fraud blast radius. No additional client or server throttling is implemented; the §4.1 resend cooldown is UX, not security.
- **Cost:** SMS verifications are billed per message (order of $0.01 US / $0.06+ Belgium-EU). Sign-ins are rare — Firebase refresh tokens keep a device signed in indefinitely until sign-out or uninstall — so family-and-friends-scale cost is cents/month; the 000 cost target ("a few euros/month") is unaffected. The region allowlist and budget alert are the guardrails.
