...
| Backend | ✅ **COMPLETE — B1–B6 all merged** (family/device/http, locations §5.1/§5.2, invites/members §3.2–3.6, locate+FCM §6/§8, history §5.3/§7.4, geofences §7). **328 unit tests, mutation 99.61%** (break 60), Azurite integration suite green. Every §11 checklist item covered. Not yet deployed (H1 pending). |
| Android / iOS | ✅ **Foundation + all feature screens merged** (A1/A2, I1/I2) — design-swappable layer (tokens→theme→stateless components), full 001 client, all screens (map/history/geofences/locate/settings/invites). Review-gated; **CI-compiled (not built locally)**: Android needs Gradle/SDK, iOS `WaldoKit` `swift build`s green (test runtime runs on `macos-14` CI). |
| web | ⬜ Not started (placeholder README) |
| Azure/Firebase provisioning | ⬜ Manual, human-run: `docs/azure-setup.md` |

## What's next (2026-07-19 — all backlog coding tasks done)

Every backlog task B1–B6 / A1–A2 / I1–I2 is **merged**. The frontier is now the **human / infra** work, none of which the dev-loop can do:
